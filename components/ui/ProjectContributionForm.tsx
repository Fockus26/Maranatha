...
export function ProjectContributionForm({
  ...
  useEffect(() => {
    const target = highlighted ? Math.min(((currentAmount + amount) / goalAmount) * 100, 100) : basePct;
  ...
  return (
    <DonationFormCard>
      <Box sx={{ display: "flex", gap: 2.5, alignItems: "center", pb: 4, mb: 4.5, borderBottom: `1px solid ${theme.palette.divider}` }}>
        <Box
          sx={{
            ...
            flexShrink: 0,
            backgroundImage: projectImageUrl ? `url(${projectImageUrl})` : undefined,
            backgroundColor: projectImageUrl ? undefined : theme.palette.primary.dark,
            backgroundSize: "cover",
            backgroundPosition: "center",
          }}
        />
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography sx={{ fontFamily: typography.fontFamily.heading, fontWeight: 600, fontSize: "13px", color: theme.palette.text.primary, mb: 0.5 }}>
            {projectTitle}
          </Typography>
      ...
      <Box sx={{ position: "relative", height: 8, borderRadius: "20px", backgroundColor: theme.palette.action.hover, overflow: "hidden", mb: 1.5 }}>
        <Box
          sx={{
            position: "absolute",
            left: 0,
            ...
            borderRadius: "20px",
            width: `${displayPct}%`,
            backgroundColor: highlighted ? theme.palette.success.main : theme.palette.secondary.main,
            transition: barTransition,
          }}
        />
      </Box>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", mb: 4.5 }}>
        <Typography
          sx={{
            fontFamily: typography.fontFamily.heading,
            fontWeight: 700,
            fontSize: highlighted ? "22px" : "15px",
            color: highlighted ? theme.palette.success.main : theme.palette.text.primary,
