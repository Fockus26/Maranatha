...
export function InstagramSection({ data }: Props): ReactElement | null {
    ...
    if (redes.length === 0) {
        throw new Error("redes cannot be empty");
    }

    const redActiva = redes.find((r) => r.id === activeRed) ?? redes[0];

    if (!redActiva) return null;
            ...
            <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-24">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {postsActivos.map((post, idx) => (
                        <a
                            key={`${activeRed}-${crypto.randomUUID()}`}
                            href={post.url}
                            target="_blank"
                ...
                <div className="flex justify-end mt-8">
                    ...
                    >
                        ...
                        <span
                            ...
                            style={{ color: activeRed === "instagram" ? "#DC2743" : "#1877F2" }}
                        >
                            open_in_new
                        </span>
                    </a>
