...
export default function InstagramSection({ data }: Props): ReactElement {
	const [activeRed, setActiveRed] = useState('instagram')
	const containerRef = useRef<HTMLDivElement>(null)

	const redActiva = redes.find((r) => r.id === activeRed) ?? redes[0]!
	...
	const postsActivos = activeRed === 'instagram' ? data.posts : data.facebookPosts
			...
			<div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-24">
				...
				<div className="flex flex-col mb-12 gap-6">
					...
					<div className="flex items-center justify-between">
						<div>
							<p className="text-primary-container font-bold text-[2rem]">{redActiva.handle}</p>
						...
						<div className="flex items-center gap-2 bg-white rounded-2xl p-2 shadow-sm border border-border">
							{redes.map((red) => (
								<button
									...
									className={`flex items-center gap-2.5 px-7 py-3.5 rounded-xl text-base font-semibold transition-all duration-300 ${activeRed === red.id
					...
					{postsActivos.map((post, idx) => (
						...
						>
							...
							<div className="absolute inset-0 bg-primary-container/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
								<span className="material-symbols-outlined text-4xl text-white select-none">favorite</span>
							</div>
						</a>
					))}
				</div>
				...
				<div className="flex justify-end mt-8">
					...
					>
						<span className="font-bold bg-clip-text text-transparent" style={{ backgroundImage: redActiva.gradient }}>
							{activeRed === 'instagram' ? 'Síguenos en Instagram' : 'Síguenos en Facebook'}
						</span>
						<span className="material-symbols-outlined text-sm" style={{ color: activeRed === 'instagram' ? '#DC2743' : '#1877F2' }}>
							open_in_new
						</span>
					</a>
				</div>
